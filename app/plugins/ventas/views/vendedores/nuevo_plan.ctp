...
<h3>ASIGNAR PLAN AL VENDEDOR</h3>
...
<?php echo $frm->create(null,array('action' => 'nuevo_plan/' . $vendedor['Vendedor']['id'], 'onsubmit' => 'return validComision()'))?>
<table class="tbl_form">
	<tr>
		<td>PLAN</td>
		<td>
			<select name="data[VendedorProveedorPlan][proveedor_plan_id]">
				<?php foreach($planes as $plan):?>
				<option value="<?php echo $plan['ProveedorPlan']['id']?>"><?php echo $plan['ProveedorPlan']['cadena']?></option>
				<?php endforeach;?>
			</select>
		</td>
	</tr>
	<tr>
		<td>VENTA MAYOR A</td><td><?php echo $frm->money('VendedorProveedorPlan.monto_venta',null,'0.00')?></td>
	</tr>
	<tr>
		<td>COMISION (%)</td><td><?php echo $frm->money('VendedorProveedorPlan.comision',null,'0.00')?></td>
	</tr>
	
</table>
<?php echo $frm->hidden('VendedorProveedorPlan.vendedor_id',array('value' => $vendedor['Vendedor']['id']))?>
