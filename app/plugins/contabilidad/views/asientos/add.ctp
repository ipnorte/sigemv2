<?php 
	if($apertura == 1):
		$action = 'apertura/' . $ejercicio['id']; 
		echo $this->renderElement('head',array('plugin' => 'config','title' => 'ASIENTO APERTURA EJERCICIO'));
		
	else:
		$action = 'add/' . $ejercicio['id']; 
		echo $this->renderElement('head',array('plugin' => 'config','title' => 'ASIENTO :: NUEVO'));
	endif;
?>
...
function validateForm(){
	var diaSel  = $('AsientoFechaDay').getValue();
	var mesSel  = $('AsientoFechaMonth').getValue();
	var anioSel = $('AsientoFechaYear').getValue();
	
	var fechaSel = diaSel + '/' + mesSel + '/' + anioSel;
	var hoy = $('AsientoFechaControl').getValue();
	if(!fechaMayor(fechaSel,hoy)){
		alert("La fecha debe ser mayor al ultimo cierre " + hoy);
		return false;
	}
	return true;
}
...
<?php // echo $form->create(null,array('name'=>'formAsiento','id'=>'formAsiento','onsubmit' => "return validateForm();",'action' => 'add/' . $ejercicio['id'] ));?>
<?php echo $form->create(null,array('name'=>'formAsiento','id'=>'formAsiento','onsubmit' => "return validateForm();",'action' => $action ));?>
...
<div class="areaDatoForm">
	 		...
	 		<div class='row'>
				<?php echo $frm->comboTipoDocumento(); ?>
	 		</div>	 		
	 		<div class='row'>
				<?php echo $frm->input('Asiento.nro_documento',array('label'=>'NRO.DOCUMENTO:','size'=>12, 'maxlenght'=>12)); ?>
	 		</div>	 		
</div>
