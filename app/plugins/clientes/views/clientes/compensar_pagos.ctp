...
<script language="Javascript" type="text/javascript">
	...
	function selSumFac(){	
		...
		for (i=1;i<=rFac;i++){
			...
			oChkCheck = document.getElementById('ClienteSaldoFac_' + i);
			oTxtImpoPagar = document.getElementById('ClienteImporteFac_' + i);
...
//		document.getElementById('ClienteImportePago').value = 0;
	...
	function selSumAnt(){	
		...
		for (i=1;i<=rAnt;i++){
			...
			oChkCheck = document.getElementById('ClienteSaldoAnt_' + i);
			oTxtImpoPagar = document.getElementById('ClienteImporteAnt_' + i);
			if (oChkCheck.checked){
				items = 1;
				strValTxt = oTxtImpoPagar.value;
				numValTxt = new Number(strValTxt);
...
<h3>COMPENSAR PAGOS</h3>
	<?php echo $frm->create(null,array('name'=>'formDetallePago','id'=>'formDetallePago','onsubmit' => "return ctrlCompensar()", 'action' => "compensar_pagos/" . $clientes['Cliente']['id'] ));
 		if($cntRenglones > 0):?>
				...
				<?php
					$i = 0;
			  		...
			  		<tr id="FAC_<?php echo $i?>">
						...
						<td>
							<input type="checkbox" name="data[Cliente][fac][check][<?php echo $i ?>]" value="<?php echo number_format(round($fPendiente["saldo"],2) * 100,0,".","")?>" id="ClienteSaldoFac_<?php echo $i?>" onclick="chkOnclickFac()"/>
							<?php // echo $frm->hidden("Cliente.fac.tipo.$i", array('value' => $fPendiente['tipo'])) ?>
						    <?php echo $frm->hidden("Cliente.fac.id.$i", array('value' => $fPendiente['id'])) ?>
						</td>
						<td align="right"><input type="hidden" name="data[Cliente][fac][importe_a_pagar][<?php echo $i ?>]" type="text" value="<?php echo number_format($fPendiente["saldo"],2,".","") ?>" id="ClienteImporteFac_<?php echo $i ?>" /></td>
			  		</tr>
			  		<?php endif;?>
		  		<?php endforeach;?>
		  		<tr>
				...
				<?php
					$i = 0;
			  		...
			  		<tr id="ANT_<?php echo $i?>">
						...
						<td>
							<input type="checkbox" name="data[Cliente][ant][check][<?php echo $i ?>]" value="<?php echo number_format(round($fPendiente["saldo"],2) * 100,0,".","")?>" id="ClienteSaldoAnt_<?php echo $i?>" onclick="chkOnclickAnt()"/>
							<?php echo $frm->hidden("Cliente.ant.tipo.$i", array('value' => $fPendiente['tipo'])) ?>
						    <?php echo $frm->hidden("Cliente.ant.id.$i", array('value' => $fPendiente['id'])) ?>
						</td>
						<td align="right"><input type="hidden" name="data[Cliente][ant][importe_a_pagar][<?php echo $i ?>]" type="text" value="<?php echo number_format($fPendiente["saldo"],2,".","") ?>" id="ClienteImporteAnt_<?php echo $i ?>" /></td>
			  		</tr>
			  		<?php endif;?>
		  		<?php endforeach;?>
		  		<tr>
		...
		<div  class="areaDatoForm">	
			...
			<table class="tbl_form">
				<tr id="fPago">
					...
					<td><?php echo $frm->calendar('Cliente.fpago',null,$fechaPago,date('Y')-1,date('Y')+1)?></td>
				</tr>
			</table>
		</div>
		
		<div style="clear: both;"></div>
		<?php echo $frm->hidden("Cliente.cliente_id", array('value' => $clientes['Cliente']['id'])) ?>
		<?php echo $frm->btnGuardarCancelar(array('URL' => '/Clientes/clientes/pendientes/' . $clientes['Cliente']['id']))?>
