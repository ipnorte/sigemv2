<?php
class ReciboDetalle extends ClientesAppModel{
	var $name = 'ReciboDetalle';
...
//	}
	...
	function detalleReciboCaja($datos){
		...
		$sql = "select global_datos.concepto_1,
                tcuota.concepto_1
                , globaldatos.concepto_1, orden_descuentos.*, orden_descuentos.numero, 
				orden_descuentos.cuotas, sum(orden_caja_cobro_cuotas.importe) as importe,
				sum(orden_caja_cobro_cuotas.importe_abonado) as importe_abonado
				,group_concat(ifnull(orden_descuento_cuotas.periodo_origen,orden_descuento_cuotas.periodo) ORDER BY ifnull(orden_descuento_cuotas.periodo_origen,orden_descuento_cuotas.periodo) ASC) as periodos
				,concat(group_concat(orden_descuento_cuotas.nro_cuota ORDER BY orden_descuento_cuotas.nro_cuota ASC),'/',orden_descuentos.cuotas) as cuota_cuotas
				...
				inner join global_datos tcuota
				on tcuota.id = orden_descuento_cuotas.tipo_cuota

                
				inner join global_datos as globaldatos
				on globaldatos.id = global_datos.concepto_2
				INNER JOIN global_datos AS tipocuota ON tipocuota.id = orden_descuento_cuotas.tipo_cuota    
...
//		if($origenFondo != MUTUALPROVEEDORID):
		...
		$sql .= " group	by orden_descuentos.id,orden_descuento_cuotas.tipo_cuota
		...
		foreach($aOrdenDescuentos as $aOrDescuento):
			...
			// $sqlCuotas = "select	OrdenDescuentoCuota.nro_cuota as nro_cuota, ifnull(OrdenDescuentoCuota.periodo_origen,OrdenDescuentoCuota.periodo) as periodo
			...
			$strDesc = $aOrDescuento[0]['cuota_cuotas'];
			...
			// $periodo = Set::extract('{n}.0.periodo', $aOrdenDescuentoCuotas);
			...
			$strPeriodo = $aOrDescuento[0]['periodos'];
			...
			$aTmpDetalle['concepto'] = 'EXPTE: ' . $aOrDescuento['orden_descuentos']['numero'] 
                                    . ' - ' 
                                    . $aOrDescuento['global_datos']['concepto_1'] .'-'. $aOrDescuento['tcuota']['concepto_1'] 
                                    . ' - ctas: ' 
                                    . $strDesc;
			if($aOrDescuento['orden_descuentos']['tipo_orden_dto'] == 'CMUTU'):
				$aTmpDetalle['concepto'] = $aOrDescuento['tipocuota']['concepto_1'] . ' - PER.: ' . $strPeriodo;
			endif; 
