...
class MutualAdicionalPendienteNoImputada extends MutualAppModel{
	
	var $name = 'MutualAdicionalPendienteNoImputada';
        var $use = 'mutual_adicional_pendientes';
    ...
    function generarAdicional($liquidacion_id,$socio_id,$codigoOrganismo,$periodo,$situacionDeuda = 'MUTUSICUMUTU',$pre_imputacion=false,$drop=TRUE){
...
//                debug($adicionalPendiente);
                ...
                if($adicional['MutualAdicional']['devengado_previo'] == 0 && !empty($adicionalPendiente)){
                    ...
                    if($adicional['MutualAdicional']['activo'] == 1){
                        ...
                        #Adrian 29/09/2016
                        ############################################################################
//                        $sql = "delete from liquidacion_cuota_noimputadas where liquidacion_id = $liquidacion_id and socio_id = $socio_id and ifnull(mutual_adicional_pendiente_id,0) <> 0";
//                        $oLC->query($sql);
...
//            debug($cuotas);
    ...
    function generarAdicionalNoImputado($liquidacion_id,$socio_id,$codigoOrganismo,$periodo,$situacionDeuda = 'MUTUSICUMUTU',$pre_imputacion=false,$drop=TRUE){
...
//        debug($parametros);
		...
		App::import('Model','Mutual.LiquidacionCuotaNoImputadaNoimputada');
		$oLC = new LiquidacionCuotaNoImputadaNoimputada();        
...
//                    debug($adp);
                    ...
                    if($this->save(array('MutualAdicionalPendiente' => $adp['MutualAdicionalPendiente']))){
                        $adp['MutualAdicionalPendiente']['id'] = $this->getLastInsertID();
                        $liqCuo = array('LiquidacionCuotaNoImputadaNoimputada' => array(
                                    'id' => 0,
                                    'liquidacion_id' => $liquidacion_id,
                                    'socio_id' => $socio_id,
                                    'persona_beneficio_id' => $adp['MutualAdicionalPendiente']['persona_beneficio_id'],
...
//                            debug($liqCuo);
                        if(!$oLC->save($liqCuo)){
                            parent::rollback();
                            $this->notificaciones = "ERROR al crear la LiquidacionCuotaNoImputadaNoimputadas de los punitorios";
                            return FALSE;
                        }
                    }else{
