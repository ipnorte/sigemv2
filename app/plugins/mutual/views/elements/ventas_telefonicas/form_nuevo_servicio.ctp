		<h3>ANEXAR SERVICIO</h3>
		<div id="mensajeNoServices" class="notices_error" style="display: none;"><strong>NO EXISTEN SERVICIOS HABILITADOS</strong></div>
		<div class="areaDatoForm">
			<?php echo $frm->create(null,array('action' => 'index/'.$persona['Persona']['id'].'/3','id' => 'NuevaOrdenServicio','onsubmit' => "return validateFormNuevoServicio();"))?>
			
			<script type="text/javascript">
			
				var adicionales = <?php echo (!empty($persona['Persona']['adicionales']) ? count($persona['Persona']['adicionales']) : 0 )?>;
				...
				Event.observe(window, 'load', function() {
					...
					if($('MutualServicioSolicitudCantidad').getValue() == 0){
						$('NuevaOrdenServicio').disable();
						$('btn_submit').disable();
						$("btn_cancel").enable();
						$("mensajeNoServices").show();
						return;
					}	
			...
			//			CuotaSocial(beneficio,getProveedorId($('ProductoMutual').getValue(),5));
				...
				function validateFormNuevoServicio(){
			...
			<table class="tbl_form">
				...
				<tr>
					<td>SERVICIO</td><td><?php echo $this->renderElement('mutual_servicios/combo_servicios',array('plugin' => 'mutual','model' => 'MutualServicioSolicitud','selected' => $this->data['SocioServicioSolicitud']['tipo_servicio_mutual_producto_id'], 'call_center' => 1))?></td>
				</tr>
				<tr>
				...
				<tr>
					<td>BENEFICIO</td><td><?php echo $this->renderElement('persona_beneficios/combo_beneficios',array('plugin' => 'pfyj','model' => 'MutualServicioSolicitud','persona_id' => $persona['Persona']['id'],'soloActivos' => 1))?></td>
				</tr>
				<tr>
			...
			<?php if(!empty($persona['Persona']['adicionales'])):?>
				<table>
					<tr>
						<th colspan="6">INCORPORAR ADICIONALES AL SERVICIO</th>
					</tr>
					<tr>
						<th></th>
						<th>DOCUMENTO</th>
				...
				<?php foreach($persona['Persona']['adicionales'] as $adicional):?>
					<tr id="<?php echo $adicional['SocioAdicional']['id']?>">
						...
						<td align="center"><?php echo $adicional['SocioAdicional']['edad']?></td>
					</tr>
					<?php $i++;?>
				<?php endforeach;?>
				</table>	
