...
<?php echo $this->requestAction('/mutual/mutual_producto_solicitudes/view/'.$mutual_producto_solicitud_id)?>
	...
	<?php echo $frm->create(null,array('name'=>'formDetallePago','id'=>'formDetallePago','onsubmit' => "return validateConfirm();", 'action' => 'pendientes_aprobar_opago'))?>

<script language="Javascript" type="text/javascript">
    function validateConfirm(){
        var msg = "APROBAR LA SOLICITUD DE CREDITO # <?php echo $solicitud['MutualProductoSolicitud']['id']?>";
...
//		debug($solicitud);
		if($solicitud['MutualProductoSolicitud']['prestamo'] == 1):
			$importePago = $solicitud['MutualProductoSolicitud']['importe_total'];
	?>
		<script language="Javascript" type="text/javascript">
			var rows = <?php echo ( isset($facturaPendiente) ? count($facturaPendiente) : 0) ?>;
			var fechaPago = "<?php echo date('Y-m-d')?>";
			var importeTotal = <?php echo  $solicitud['MutualProductoSolicitud']['importe_total'] ?>;
			...
			{
//				var fecha_operacion = $F('MovimientoFechaOperacionYear') + '-' + $F('MovimientoFechaOperacionMonth') + '-' + $F('MovimientoFechaOperacionDay');
...
//						  				ocultarOptionFPago();
		...
		<div style="clear: both;"></div>
			<?php echo $frm->hidden("Movimiento.socio_id", array('value' => $solicitud['MutualProductoSolicitud']['socio_id'])) ?>
	...
	<?php echo $frm->btnGuardarCancelar(array('TXT_GUARDAR' => 'APROBAR ORDEN DE CONSUMO / SERVICIO','URL' => ( empty($fwrd) ? "/mutual/mutual_producto_solicitudes/pendientes_aprobar_opago" : $fwrd) ))?>
<?php 
//debug($solicitud);
?>