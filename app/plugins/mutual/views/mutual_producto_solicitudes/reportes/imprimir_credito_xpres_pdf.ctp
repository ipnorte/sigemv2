...
if(!empty($orden['MutualProductoSolicitud']['detalle_calculo_plan'])){
    ...
    $PDF->ocom_imprime_mutuo_ryvsa($orden,$TEXTO, true, false, true);
}

$PDF->hojaEnBlanco();
$TEXTO = getPlantillaTextoContrato();
$PDF->ocom_imprime_mutuo_ryvsa($orden,$TEXTO,false, false, true);
...
$gtoAdminOwner = "MARGEN COMERCIAL S.A.";

$PDF->autorizacionCobranzaDebitoDirectoGeneral($orden, $gtoAdminOwner, NULL);
