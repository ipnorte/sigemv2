...
$oPHPExcel->getActiveSheet()->setCellValue('B1',(isset($liquidacion['Liquidacion']['codigo_organismo']) ? $util->globalDato($liquidacion['Liquidacion']['codigo_organismo']) : ' ** GENERAL **')  . " | PERIODO: " . (isset($liquidacion['Liquidacion']['periodo']) ? $util->periodo($liquidacion['Liquidacion']['periodo']) : $util->periodo($periodo)));

$oPHPExcel->getActiveSheet()->setTitle(($procesarSobrePreImputacion == 1 ? "PRE-IMPUTACION" : "IMPUTACION"));
$offSet = 5;
$oPHPExcel->getActiveSheet()->setCellValue('B'.($offSet - 2),'RESUMEN '.($procesarSobrePreImputacion == 1 ? "PRE-IMPUTACION" : "IMPUTACION").' PROVEEDORES');
