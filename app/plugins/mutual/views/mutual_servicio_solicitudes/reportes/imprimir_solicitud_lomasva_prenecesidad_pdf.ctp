...
//	$PDF->Cell(0,0,'Sres. ComisiÃ³n Directiva:',0);
//	$PDF->Ln(3);
	$PDF->MultiCell(0,0,"Sres. ComisiÃ³n Directiva:\nPor la presente, solicito a Uds. la incorporaciÃ³n de las siguientes personas al servicio de " . $solicitud['MutualServicioSolicitud']['mutual_proveedor_servicio']. ", en las condiciones pactadas por la ".strtoupper(Configure::read('APLICACION.nombre_fantasia'))." con dicha Empresa, las que declaro conocer y aceptar.\n",0,'J');
...
//$PDF->MultiCell(0,0,"IMPORTANTE: La entidad no abonarÃ¡ el excedente de las compras cuyo valor supere el importe autorizado. La presente orden carece de validez si no se acompaÃ±a con el D.N.I. del titular.\n",0,'J');
$PDF->Ln(8);
$PDF->linea[1] = array(
			'posx' => 20,
			'ancho' => 40,
...
$PDF->linea[6] = array(
			'posx' => 133,
			'ancho' => 10,
			'texto' => $solicitud['MutualServicioSolicitud']['cuotas'],
			'borde' => 'LTBR',
			'align' => 'R',
			'fondo' => 1,
			'style' => 'B',
...
$PDF->linea[8] = array(
			'posx' => 168,
			'ancho' => 25,
			'texto' => $util->nf($solicitud['MutualServicioSolicitud']['importe_cuota']),
			'borde' => 'LTBR',
			'align' => 'R',
			'fondo' => 1,
			'style' => 'B',
...
//	$STR = "PAGARE SIN PROTESTO (Art. 50 D.Ley 5965/63) a $pagareNombre o a su orden la cantidad de pesos ".$solicitud['MutualServicioSolicitud']['importe_mensual_total_letras']." ($ ".$util->nf($solicitud['MutualServicioSolicitud']['importe_mensual_total']).") por igual valor recibido en _______________________________ a mi entera satisfacción, pagadero en $pagareDomici. Conforme a lo dispuesto por el Art. 36 del D.Ley 5965 se amplia el plazo de la presente a cuatro años.\n";
//	$STR = utf8_encode($STR);
//	$PDF->MultiCell(0,0,$STR,0,'J');
//	$PDF->Ln(10);
//	$PDF->firmaSocio();
