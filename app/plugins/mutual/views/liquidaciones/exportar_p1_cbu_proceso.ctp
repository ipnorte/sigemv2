...
<div class="areaDatoForm">
	...
	<table>
		...
		<?php foreach($datos['LiquidacionSocio']['turno_pago'] as $codigo => $turno):?>
			<?php 
		...
		<tr>
			<td colspan="4"><strong><?php echo $util->banco($datos['LiquidacionSocio']['banco_intercambio'])?></strong></td>
			<td align="right"><strong><?php echo $util->armaFecha($datos['LiquidacionSocio']['fecha_debito'])?></strong></td>
		</tr>
...
echo $this->renderElement('show',array(
										'plugin' => 'shells',
										'process' => 'genera_archivo',
										'accion' => '.mutual.liquidaciones.exportar2.'.$liquidacion['Liquidacion']['id'].'.1',
										'target' => '',
										'btn_label' => '',
										'titulo' => 'GENERAR DISKETTE',
										'subtitulo' => $util->banco($datos['LiquidacionSocio']['banco_intercambio'])." | FECHA DEBITO: ". $util->armaFecha($datos['LiquidacionSocio']['fecha_debito']) . " | TOTAL $ ". $util->nf($ACU_IMP),
