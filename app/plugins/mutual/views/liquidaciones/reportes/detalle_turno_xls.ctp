...
$oPHPExcel->getActiveSheet()->setCellValue("C4","TEL MOVIL");
$oPHPExcel->getActiveSheet()->setCellValue("D4","TEL FIJO");
$oPHPExcel->getActiveSheet()->setCellValue("E4","TEL REF");

$oPHPExcel->getActiveSheet()->setCellValue("F4","REG");
$oPHPExcel->getActiveSheet()->setCellValue("G4","BANCO");
$oPHPExcel->getActiveSheet()->setCellValue("H4","SUCURSAL");
$oPHPExcel->getActiveSheet()->setCellValue("I4","CUENTA");
$oPHPExcel->getActiveSheet()->setCellValue("J4","CBU");
$oPHPExcel->getActiveSheet()->setCellValue("K4","SALDO");
$oPHPExcel->getActiveSheet()->setCellValue("L4","DEBITO");
...
if(!empty($socios)):
	...
	foreach($socios as $socio):
		...
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$i)->setValueExplicit(utf8_encode($socio['LiquidacionSocio']['apenom']));
                
                $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$i)->setValueExplicit($socio['LiquidacionSocio']['persona_telefono_movil']);
                $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$i)->setValueExplicit($socio['LiquidacionSocio']['persona_telefono_fijo']);
                $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$i)->setValueExplicit($socio['LiquidacionSocio']['persona_telefono_referencia']);
                
                
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$i)->setValue($socio['LiquidacionSocio']['registro']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(6,$i)->setValueExplicit($socio['LiquidacionSocio']['banco']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$i)->setValue($socio['LiquidacionSocio']['sucursal']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(8,$i)->setValue($socio['LiquidacionSocio']['nro_cta_bco']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(9,$i)->setValueExplicit($socio['LiquidacionSocio']['cbu']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(10,$i)->setValue($socio['LiquidacionSocio']['saldo_actual']);
                $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(11,$i)->setValue($socio['LiquidacionSocio']['importe_adebitar']);
		$n++;
		$i++;
	endforeach;
...
if(!empty($socios_error_cbu)):
	...
	foreach($socios_error_cbu as $socio):
		...
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$i)->setValueExplicit($socio['LiquidacionSocio']['registro']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$i)->setValueExplicit($util->banco($socio['LiquidacionSocio']['banco_id']));
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$i)->setValue($socio['LiquidacionSocio']['sucursal']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(6,$i)->setValue($socio['LiquidacionSocio']['nro_cta_bco']);
		$oPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$i)->setValue($socio['LiquidacionSocio']['cbu']);
