...
###########################################################################################################################
#	IMPRIMO LOS DATOS DEL ANALISIS DEL DISKETTE
###########################################################################################################################
//$PDF->linea[0] = array(
//	'posx' => $L1[0],
//	'ancho' => 190,
//	'texto' => "FECHA DEBITO: " . $diskette['fecha_debito'],
//	'borde' => 'TBLR',
//	'align' => 'L',
//	'fondo' => 0,
//	'style' => 'B',
...
foreach($datos['info_procesada_by_turno'] as $codigoTurno => $turno):
	...
	foreach($turno['registros'] as $socio):
		...
		$ACU_IMPO += $socio['LiquidacionSocioNoimputada']['importe_adebitar'];
		$ACU_IMPODTO += $socio['LiquidacionSocioNoimputada']['importe_dto'];
		$ACU_REGISTROS++;
		$reg++;
		$linea++;
		...
		$PDF->linea[1] = array(
			'posx' => $L1[1],
			'ancho' => $W1[1],
			'texto' => substr($socio['LiquidacionSocioNoimputada']['documento'] ." - ". utf8_encode($socio['LiquidacionSocioNoimputada']['apenom']),0,55),
			'borde' => '',
			'align' => 'L',
			'fondo' => 0,
			'style' => '',
		...
		$PDF->linea[2] = array(
			'posx' => $L1[2],
			'ancho' => $W1[2],
			'texto' => $socio['LiquidacionSocioNoimputada']['registro'],
			'borde' => '',
			'align' => 'C',
			'fondo' => 0,
			'style' => '',
		...
		$PDF->linea[3] = array(
			'posx' => $L1[3],
			'ancho' => $W1[3],
			'texto' => $socio['LiquidacionSocioNoimputada']['sucursal']."-".$socio['LiquidacionSocioNoimputada']['nro_cta_bco'],
			'borde' => '',
			'align' => 'C',
			'fondo' => 0,
			'style' => '',
		...
		$PDF->linea[4] = array(
			'posx' => $L1[4],
			'ancho' => $W1[4],
			'texto' => $socio['LiquidacionSocioNoimputada']['cbu'],
			'borde' => '',
			'align' => 'C',
			'fondo' => 0,
			'style' => '',
		...
		$PDF->linea[5] = array(
			'posx' => $L1[5],
			'ancho' => $W1[5],
			'texto' => $util->nf($socio['LiquidacionSocioNoimputada']['importe_adebitar']),
			'borde' => '',
			'align' => 'R',
			'fondo' => 0,
			'style' => '',
	...
	$aACUM_DISK[$codigoTurno]['registros'] = $reg;
	$aACUM_DISK[$codigoTurno]['importe_adebitar'] = $ACU_IMPO;
...
foreach($datos['info_procesada_by_turno'] as $codigoTurno => $turno):
	...
	$PDF->linea[0] = array(
		'posx' => $L1[0],
		'ancho' => $W1[0],
		'texto' => $aACUM_DISK[$codigoTurno]['registros'],
		'borde' => '',
		'align' => 'L',
		'fondo' => 0,
		'style' => '',
	...
	$PDF->linea[2] = array(
		'posx' => $L1[2],
		'ancho' => $W1[2],
		'texto' => $util->nf($aACUM_DISK[$codigoTurno]['importe_adebitar']),
		'borde' => '',
		'align' => 'R',
		'fondo' => 0,
		'style' => '',
...
if(!empty($errores)):
	...
	$regT = $ACU_IMPOT = $ACU_IMPODTOT = $ACU_REGISTROST = 0;
	$aACUM_DISK = array();
	$aACUM_DISK_TOTALES = array();
	...
	foreach($datos['info_procesada_by_turno'] as $codigoTurno => $turno):
		...
		if(!empty($turno['errores'])):
			...
			foreach($turno['errores'] as $socio):
				...
				$PDF->linea[0] = array(
					'posx' => $L1[0],
					'ancho' => $W1[0],
					'texto' => substr($socio['LiquidacionSocioNoimputada']['documento'] ." - ". $socio['LiquidacionSocioNoimputada']['apenom'],0,55),
					'borde' => '',
					'align' => 'L',
					'fondo' => 0,
					'style' => '',
				...
				$PDF->linea[5] = array(
					'posx' => $L1[5],
					'ancho' => $W1[5],
					'texto' => $socio['LiquidacionSocioNoimputada']['error_intercambio'],
					'borde' => '',
					'align' => 'L',
					'fondo' => 0,
					'style' => '',
