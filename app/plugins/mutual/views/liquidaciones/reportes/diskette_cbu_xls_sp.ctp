...
//        debug($turno['descripcion']);
        ...
        list($apellido,$nombre) = explode(",",utf8_encode($socio['LiquidacionSocio']['apenom']));
        
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$i)->setValueExplicit($socio['GlobalDato']['concepto_1']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$i)->setValueExplicit($socio['LiquidacionSocio']['documento']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$i)->setValueExplicit($socio['LiquidacionSocio']['cuit_cuil']);
        ...
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$i)->setValueExplicit($socio['LiquidacionSocio']['socio_id']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(6,$i)->setValueExplicit($socio['LiquidacionSocio']['cbu']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$i)->setValueExplicit($socio['LiquidacionSocio']['sucursal']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(8,$i)->setValueExplicit($socio['LiquidacionSocio']['nro_cta_bco']);
        $oPHPExcel->getActiveSheet()->getCellByColumnAndRow(9,$i)->setValue($socio['LiquidacionSocio']['importe_adebitar']);
