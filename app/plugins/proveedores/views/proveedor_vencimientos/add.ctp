...
<?php echo $form->create(null,array('action' => 'add/'. $codigo_organismo.'/'.$mes));?>
<div class="areaDatoForm">
	<h3 style="border-bottom: 1px solid;"><?php echo $this->requestAction('/config/global_datos/valor/'.$codigo_organismo.'/concepto_1')?> :: <?php echo $util->mesToStr($mes,true)?></h3>
	<div class="row">
		<table cellpadding="0" cellspacing="0">
			...
			<tr>
				<td colspan="2">
					...
					<?php echo $this->renderElement('proveedor/grilla_checks',array('modelo' => 'ProveedorVecimiento','soloActivos' => 0, 'plugin' => 'proveedores'))?>
				</td>
			</tr>
...
<?php echo $frm->hidden('ProveedorVencimiento.codigo_organismo',array('value'=>$codigo_organismo))?>
<?php echo $frm->hidden('ProveedorVencimiento.mes',array('value'=>$mes))?>
<?php echo $frm->btnGuardarCancelar(array('TXT_GUARDAR' => 'GUARDAR','URL' => ( empty($fwrd) ? "/proveedores/proveedor_vencimientos/index?data[ProveedorVencimiento][codigo_organismo]=".$codigo_organismo."&data[ProveedorVencimiento][mes][month]=".$mes : $fwrd) ))?>