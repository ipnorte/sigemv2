...
<h3>ADMINISTRACION DE PLANES :: NUEVO PLAN</h3>
<script type="text/javascript">

Event.observe(window, 'load', function(){
	disableParamsCalculo();
...
<?php echo $frm->create(null,array('action' => 'nuevo_plan/' . $proveedor['Proveedor']['id']))?>
<table class="tbl_form">
	<tr>
		<td>DENOMINACION</td><td><?php echo $frm->input('ProveedorPlan.descripcion',array('size'=>60,'maxlength'=>100)); ?></td>
	</tr>
	<tr>
		<td>TIPO PRODUCTO</td>
		<td>
    ...
    <tr>
            ...
            <td>
                <div class="areaDatoForm2">
                    <table class="tbl_form">
                        ...
                        <?php echo $frm->input('ProveedorPlan.metodo_calculo',array('label' => 'METODO DE CALCULO','type' => 'select','empty' => true,'options' => $metodos, 'value' => ''))?></td>
                        <td>
                        <?php echo $frm->money('ProveedorPlan.tna','T.N.A. (TASA NOMINAL ANUAL - %)','',false,6,6)?>
                        </td>
                        <td>
                        <?php echo $frm->money('ProveedorPlan.iva','I.V.A. (%)','',false,4,4)?>
                        </td>
                        </tr>
                    </table> 
                    <table class="tbl_form"> 
        <tr>
            ...
            <?php echo $this->renderElement('global_datos/combo_cuotas_puntuales',array(
																					...
																					'disable' => false,
																					'empty' => true,
																					'selected' => "MUTUTCUOGOTO",
					))?>
            </td>
            <td>
                <?php echo $frm->money('ProveedorPlan.gasto_admin','%','',false,6,6)?>
            </td>
            <td>
            <?php echo $frm->input('ProveedorPlan.gasto_admin_base_calculo',array('label' => 'BASE DE CALCULO','type' => 'select', 'options' => $criterios, 'value' => ''))?>
            </td>
        ...
        <td>
            <?php echo $this->renderElement('global_datos/combo_cuotas_puntuales',array(
																					...
																					'disable' => false,
																					'empty' => true,
																					'selected' => "MUTUTCUOSELL",
					))?>
            </td>        
            <td>
                <?php echo $frm->money('ProveedorPlan.sellado','%','',false,6,6)?>
            </td>
            <td>
            <?php echo $frm->input('ProveedorPlan.sellado_base_calculo',array('label' => 'BASE DE CALCULO','type' => 'select', 'options' => $criterios, 'value' => ''))?>
            </td>        
        </tr>       
    </table>                     
                </div>
    ...
    <tr>
        ...
        <td>
            <?php echo $frm->money('ProveedorPlan.interes_moratorio','INTERES MORATORIO %','0.00',false,6,6)?>
            <?php echo $frm->money('ProveedorPlan.costo_cancelacion_anticipada','COSTO CANCELACION ANTICIPADO %','0.00',false,6,6)?>
        </td>
    </tr>

        <?php if(isset($_SESSION['MUTUAL_INI']['general']['modulo_ayuda_economica']) && $_SESSION['MUTUAL_INI']['general']['modulo_ayuda_economica'] == 1):?>
        <tr>
            <td>AYUDA ECONOMICA (Res. No1418/03 INAES)</td><td><?php echo $frm->input('ProveedorPlan.ayuda_economica'); ?></td>
        </tr>
        <?php endif;?>
	<tr>
		<td>ORGANISMO</td>
		...
		echo $this->renderElement('global_datos/grilla_checks',array(
			...
			'header' => false,
			'selected' => (isset($this->data['ProveedorPlan']['organismos']) ? $this->data['ProveedorPlan']['organismos'] : array()),
		));
		?>
		</td>
	</tr>
		...
		echo $this->renderElement('global_datos/combo_global',array(
			...
			'empty' => false,
			'selected' => (isset($this->data['ProveedorPlan']['modelo_solicitud_codigo']) ? $this->data['ProveedorPlan']['modelo_solicitud_codigo'] : NULL),
		));
		?>                 
            </td>
        </tr>
		...
		echo $this->renderElement('global_datos/grilla_checks',array(
			...
			'header' => false,
			'selected' => (isset($this->data['ProveedorPlan']['anexos']) ? $this->data['ProveedorPlan']['anexos'] : array()),
		));
		?>                
            </td>
        </tr>
        <tr>
            ...
            echo $this->renderElement('global_datos/grilla_checks',array(
                    ...
                    'header' => false,
                    'selected' => (isset($this->data['ProveedorPlan']['documentos']) ? $this->data['ProveedorPlan']['documentos'] : array()),
            ));
            ?>
            </td>
	</tr>
...
<?php echo $frm->hidden('ProveedorPlan.proveedor_id',array('value' => $proveedor['Proveedor']['id'])); ?>
