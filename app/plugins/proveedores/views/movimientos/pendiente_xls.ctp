...
$oPHPExcel->getActiveSheet()->setCellValue('A1',$proveedores['Proveedor']['razon_social']);
$oPHPExcel->getActiveSheet()->setCellValue('B1','CUENTA CORRIENTE PENDIENTE DE PAGO');

$oPHPExcel->getActiveSheet()->setTitle('PENDIENTE DE PAGO');
$offSet = 5;
$oPHPExcel->getActiveSheet()->setCellValue('B'.($offSet - 2),'PENDIENTE DE PAGO');
$oPHPExcel->getActiveSheet()->getStyle('B'.($offSet - 2))->getFont()->setBold(true);
if(!empty($aPendientes)):
	$i=0;
	foreach ($aPendientes[0] as $field => $value) {
