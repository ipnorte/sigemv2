<?php
if($tipo == 7):
	$txtHeader = 'CONFIGURACION CONCEPTOS DE CAJA :: ALTA CONCEPTO';
else:
	$txtHeader = 'CONFIGURACION CONCEPTOS DE BANCO :: ALTA CONCEPTO';
endif;
?>
...
<!--	
		<tr>
			...
			<td><?php echo $frm->input('Asiento.descripcionAproxima',array('label'=>'','size'=>50,'maxlenght'=>100, 'value' => $this->data['BancoConcepto']['descripcionAproxima'])); ?>
		...
		<tr>
			...
			<td>
				<input type="text" id="descripcionCuenta" disabled="disabled" size="50"/>
			</td>
		</tr>				
		<tr>
...
-->
		<tr>
			...
			<td><input type="radio" name="data[BancoConcepto][debe_haber]" id="ConceptoAccion_a" value="0"/>INGRESO</td>
		</tr>
		<tr>
			<td></td>
			<td><input type="radio" name="data[BancoConcepto][debe_haber]" id="ConceptoAccion_b" value="1"/>EGRESO</td>
		</tr>
	</table>
</div>
<?php echo $frm->hidden('BancoConcepto.tipo', array('value' => $tipo)); ?>
<?php echo $frm->btnGuardarCancelar(array('URL' => '/cajabanco/banco_conceptos/index/' . $tipo))?>