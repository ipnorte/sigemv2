...
VIEW `v_credito_solicitud_estados` AS
    (SELECT 
    ...
    FROM
        `v_global_datos`
    WHERE
        ((`v_global_datos`.`ID` LIKE 'MUTUESTA%')
            AND (`v_global_datos`.`ID` <> 'MUTUESTA'))
    ORDER BY `v_global_datos`.`ENTERO_1` , `v_global_datos`.`CONCEPTO_1`)