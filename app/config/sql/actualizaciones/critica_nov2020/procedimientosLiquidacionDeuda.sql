...
DELIMITER $$
CREATE DEFINER=CURRENT_USER FUNCTION `FX_CALCULA_ADICIONAL`() RETURNS int
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER FUNCTION `FX_CALCULA_CUOTA_SERVICIO`(
vORDEN_ID INT(11),
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER FUNCTION `FX_CALCULA_CUOTA_SOCIAL`(
vSOCIO_ID INT(11),
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER FUNCTION `FX_CALCULAR_SALDO_CUOTA`(`vCUOTA_ID` INT(11), `vPREIMPUTADO` BOOLEAN) RETURNS decimal(10,2)
BEGIN
...
DELIMITER $$
CREATE DEFINER=`iutumipnorte_sigemdb`@`%` FUNCTION `FX_GENERAR_DISKETTE_BANCO`(
...
DROP PROCEDURE IF EXISTS `SP_LIQUIDA_DEUDA_REGISTRO_RENUMERA`;
...
DROP PROCEDURE IF EXISTS `SP_LIQUIDA_GTO_BANCARIOS`;
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_ADICIONALES`(IN `vLIQUIDACION_ID` INT(11), IN `vSOCIO_ID` INT(11))
BEGIN
DECLARE l_last_row INT DEFAULT 0;
DECLARE vPROVEEDOR_ID INT(11) DEFAULT 0;
DECLARE vIMPUTAR_PROVEEDOR_ID INT(11) DEFAULT 0;
DECLARE vTIPO CHAR(1) CHARACTER SET utf8 COLLATE utf8_general_ci;
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_CUOTA_SERVICIOS`(
vPERIODO VARCHAR(6) CHARACTER SET utf8 COLLATE utf8_general_ci,
vORGANISMO VARCHAR(12) CHARACTER SET utf8 COLLATE utf8_general_ci
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_CUOTA_SOCIAL`(IN `vPERIODO` VARCHAR(6), IN `vORGANISMO` VARCHAR(12), IN `vTIPO_PRODUCTO` VARCHAR(12), IN `vTIPO_CUOTA` VARCHAR(12), IN `vTIPO_ORDEN` VARCHAR(12), IN `vTIPO_SITUACION` VARCHAR(12), IN `vSOLO_DEUDA` BOOLEAN)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA`(
...
IF vSOCIO_ID IS NULL THEN
	update liquidacion_intercambios set procesado = 0 where liquidacion_id = vLIQUIDACION_ID;
END IF;
...
ELSE FALSE END
and find_in_set(IFNULL(so.calificacion,'MUTUCALINORM'),@calificaciones);
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_ANSES_GENERAL`(
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_ACUERDO_DEBITO`(
...
DECLARE vSALDO_RESTO DECIMAL(10,2);
...
select codigo_organismo,periodo into @ORGANISMO,@PERIODO from liquidaciones where id = vLIQUIDACION_ID;
select ifnull(decimal_1,0) into vTOPE_POR_REGISTRO from global_datos where id = @ORGANISMO;
...
FETCH c_beneficios_acuerdo INTO vSOCIO_ID,vBENEFICIOID,vBANCOID,vSUCURSAL,vTIPOCTABCO,vNROCTABCO,vCBU,vTDOC,vNDOC,vAPENOM,vPERSONAID,vCUITCUIL,vCODEMPRE,vCODREPA,vTURNOPAGO,vMONTO_ACUERDO,vMONTO_MAX_DTO_BENEFICIO,vSALDO_ACTUAL;
		...
		persona_beneficio_id = vBENEFICIOID;       
        ...
        IF vMONTO_MAX_DTO_BENEFICIO > vTOPE_POR_REGISTRO THEN
			SET vMONTO_MAX_DTO_BENEFICIO = vTOPE_POR_REGISTRO;
        END IF;
        
        -- SELECT vSOCIO_ID,vBENEFICIOID,vTOPE_POR_REGISTRO,vMONTO_MAX_DTO_BENEFICIO,vSALDO_ACTUAL,vSALDO;
        
        SET @SALDORESTANTE = vSALDO;
        ...
        IF vMONTO_MAX_DTO_BENEFICIO <> 0 THEN
            ...
            SET @REG = IFNULL(IF(vMONTO_ACUERDO <> 0,vMONTO_ACUERDO,vSALDO_ACTUAL) DIV vMONTO_MAX_DTO_BENEFICIO,0);
			...
			WHILE @N <= @REG DO
                ...
                */
                ...
                SET @SALDORESTANTE = @SALDORESTANTE - @IMPO_DEBITO;
                
				SET @VALORESINSERT = CONCAT(IF(@VALORESINSERT = '','',CONCAT(@VALORESINSERT,',')),'(',@REGISTRO,',',vLIQUIDACION_ID,',',vSOCIO_ID,',',vBENEFICIOID,',',QUOTE(@ORGANISMO),
				',',QUOTE(vBANCOID),',',QUOTE(vSUCURSAL),',',QUOTE(vTIPOCTABCO),',',QUOTE(vNROCTABCO),',',QUOTE(vCBU),','
				,QUOTE(vTDOC),',',QUOTE(vNDOC),',',QUOTE(vAPENOM),
				',',vPERSONAID,',',QUOTE(vCUITCUIL),',',QUOTE(vCODEMPRE),',',QUOTE(vCODREPA),',',QUOTE(vTURNOPAGO),',',0
				,',',@IMPO_DEBITO,',',@IMPO_DEBITO,',',QUOTE(concat('*** CON ACUERDO DE DEBITO ', vSALDO ,' | FRACCION = ',@IMPO_DEBITO,' ***')),')');                
				
                SET @N = @N + 1;
                
                -- select concat(QUOTE(vSALDO_ACUMULADO),@IMPO_DEBITO,@SALDORESTANTE);
                
			END WHILE;
            
            IF @SALDORESTANTE > 0 THEN

				SET @IMPO_DEBITO = @SALDORESTANTE;
				SET @VALORESINSERT = CONCAT(IF(@VALORESINSERT = '','',CONCAT(@VALORESINSERT,',')),'(',@REGISTRO,',',vLIQUIDACION_ID,',',vSOCIO_ID,',',vBENEFICIOID,',',QUOTE(@ORGANISMO),
				',',QUOTE(vBANCOID),',',QUOTE(vSUCURSAL),',',QUOTE(vTIPOCTABCO),',',QUOTE(vNROCTABCO),',',QUOTE(vCBU),','
				,QUOTE(vTDOC),',',QUOTE(vNDOC),',',QUOTE(vAPENOM),
				',',vPERSONAID,',',QUOTE(vCUITCUIL),',',QUOTE(vCODEMPRE),',',QUOTE(vCODREPA),',',QUOTE(vTURNOPAGO),',',0
       ...
       ELSE 
		...
		SET @VALORESINSERT = CONCAT(
        @VALORESINSERT
        ,IF(@VALORESINSERT = '','',CONCAT(@VALORESINSERT,','))
			,'(',@REGISTRO,',',vLIQUIDACION_ID,',',vSOCIO_ID,',',vBENEFICIOID,',',QUOTE(@ORGANISMO),','
			,QUOTE(vBANCOID),',',QUOTE(vSUCURSAL),',',QUOTE(vTIPOCTABCO),',',QUOTE(vNROCTABCO),','
			,QUOTE(vCBU),',',QUOTE(vTDOC),',',QUOTE(vNDOC),',',QUOTE(vAPENOM),',',vPERSONAID,','
			,QUOTE(vCUITCUIL),',',QUOTE(vCODEMPRE),',',QUOTE(vCODREPA),',',QUOTE(vTURNOPAGO),','
			,0,',',vMONTO_ACUERDO,',',vMONTO_ACUERDO,',',QUOTE(concat('*** CON ACUERDO DE DEBITO ', vMONTO_ACUERDO ,' ***')),')'
			);                
       END IF;

		-- select concat(QUOTE(vMONTO_ACUERDO),vMONTO_ACUERDO);
...
IF @VALORESINSERT <> '' THEN
	...
	PREPARE stmt_insert FROM @INSERTLS;
	EXECUTE stmt_insert;
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_MORA_GENERAL`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
-- set @periodo = DATE_FORMAT(DATE_SUB(STR_TO_DATE(CONCAT('202101','01'),'%Y%m%d'), interval 1 month),'%Y%m');
...
c1_loop: LOOP
    ...
    -- select vSOCIO_ID,vBENEFICIOID,vBANCOID,vSUCURSAL,vTIPOCTABCO,vNROCTABCO,vCBU,vTDOC,vNDOC,vAPENOM,vPERSONAID,vCUITCUIL,vCODEMPRE,vCODREPA,vTURNOPAGO,vIMPORTE_DEBITO,vFORMULA,vULTIMO_COBRO,vSALDO_ACTUAL;
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_MORA_SPARAM`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_PERIODO_CONSOLIDA_MORA`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_PERIODO_DISCRI_PERM`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CBU_PERIODO_GENERAL`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
c1_loop: LOOP
    ...
    WHILE @REGISTRO <= @N DO
        ...
        '\',\'',vBANCOID,'\',\'',vSUCURSAL,'\',\'',vTIPOCTABCO,'\',\'',vNROCTABCO,'\',\'',vCBU,'\',\''
        ,vTDOC,'\',\'',vNDOC,'\',\'',vAPENOM,
        '\',',vPERSONAID,',\'',vCUITCUIL,'\',\'',vCODEMPRE,'\',\'',vCODREPA,'\',\'',vTURNOPAGO,'\',',1
        ,',',vTOPE_POR_REGISTRO,',',vTOPE_POR_REGISTRO,',\'',concat(vFORMULA,'\n*** MAXIMO POR REGISTRO ',vTOPE_POR_REGISTRO,' ***'),'\')');  
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_CJPC_GENERAL`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_REGISTRO_RENUMERA`(
vLIQUIDACION_ID INT(11),
IN vSOCIO_ID INT(11)
)
BEGIN
...
DECLARE cursor_socio CURSOR FOR 
SELECT socio_id FROM liquidacion_socios where liquidacion_id = vLIQUIDACION_ID
AND IF(vSOCIO_ID IS NULL,TRUE,socio_id = vSOCIO_ID) group by socio_id;

DECLARE CONTINUE HANDLER FOR NOT FOUND SET l_last_row=1;
OPEN cursor_socio;
c1_loop: LOOP

	FETCH cursor_socio INTO vSOCIOID;
	IF (l_last_row = 1) THEN
		LEAVE c1_loop; 
	END IF;    
       
    update liquidacion_socios, (SELECT @pid := 0) r 
    set registro = @pid := @pid + 1
    where liquidacion_id = vLIQUIDACION_ID
    and socio_id = vSOCIOID;   
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_SCORING`(IN
vLIQUIDACION_ID INT(11),
vSOCIO_ID INT(11)
)
BEGIN
/*
no se usa mas ahora va el SP_LIQUIDA_DEUDA_SOCIOS_SCORING
...
/* EL SCORE CALCULARLO COMO PORCENTAJE DE LO EFECTIVAMENTE COBRADO 1 A 100*/

END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_SOCIOS_SCORING`(
vLIQUIDACION_ID INT(11),
vSOCIO_ID INT(11)
)
BEGIN
...
sum(saldo_actual) as saldo_actual, 0 as riesgo
, CEILING((sum(importe_debitado)/sum(saldo_actual)) * 100) as score
from liquidacion_cuotas lc2 where liquidacion_id = vLIQUIDACION_ID
AND IF(vSOCIO_ID IS NULL,TRUE,socio_id = vSOCIO_ID)
group by socio_id;
...
-- select CEILING((importe_debitado/saldo_actual) * 100) from tmp_scoring;
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_DEUDA_TOTALIZADOR`(
IN vLIQUIDACION_ID INT(11)
)
BEGIN
...
select sum(importe_debitado) into @IMPOCOBRADO from liquidacion_socio_rendiciones
where liquidacion_id = vLIQUIDACION_ID and indica_pago = 1;

select sum(importe_debitado) into @IMPONOCOBRADO from liquidacion_socio_rendiciones
where liquidacion_id = vLIQUIDACION_ID and indica_pago = 0;

select sum(importe_debitado) into @IMPUTADO from liquidacion_cuotas
where liquidacion_id = vLIQUIDACION_ID;

UPDATE liquidaciones 
set 
cuota_social_vencida = ifnull(@CUOTA_SOCIAL_VENCIDA,0),
cuota_social_periodo = ifnull(@CUOTA_SOCIAL_NOVENCIDA,0),
deuda_periodo = ifnull(@DEUDA_PERIODO,0),
deuda_vencida = ifnull(@DEUDA_VENCIDA,0),
total_vencido = ifnull(@CUOTA_SOCIAL_VENCIDA,0) + ifnull(@DEUDA_VENCIDA,0),
total_periodo = ifnull(@CUOTA_SOCIAL_NOVENCIDA,0) + ifnull(@DEUDA_PERIODO,0),
total = total_vencido + total_periodo,
importe_cobrado = ifnull(@IMPOCOBRADO,0),
importe_no_cobrado = ifnull(@IMPONOCOBRADO,0),
importe_imputado = ifnull(@IMPUTADO,0),
importe_reintegro = importe_cobrado - importe_imputado,
en_proceso = FALSE,
bloqueada = FALSE,
cerrada = TRUE                             
...
valor,tipo_cuota into @PROVEEDOR,@VALOR,@TIPOCUOTA
from mutual_adicionales
where codigo_organismo = @ORGANISMO and valor > 0
...
DELIMITER $$
CREATE DEFINER=CURRENT_USER PROCEDURE `SP_LIQUIDA_PUNITORIOS`(IN `vLIQUIDACION_ID` INT(11), IN `vSOCIO_ID` INT(11))
BEGIN
