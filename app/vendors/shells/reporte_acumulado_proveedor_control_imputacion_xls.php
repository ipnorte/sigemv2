 ...
 * /usr/bin/php5 /home/adrian/Trabajo/www/sigemv2/cake/console/cake.php reporte_acumulado_proveedor_control_imputacion_xls 40349 -app /home/adrian/Trabajo/www/sigemv2/app/
 * 
 ...
 *
 */
class ReporteAcumuladoProveedorControlImputacionXlsShell extends Shell {
	...
	function main() {
		...
		$codigo_organismo = $asinc->getParametro('txt2');
		$proveedor_id = $asinc->getParametro('p4');
		
                
//                echo base64_decode('YToxMDp7czoxMjoiTVVUVUNPUkcyMjAyIjtzOjEyOiJNVVRVQ09SRzIyMDIiO3M6MTI6Ik1VVFVDT1JHMjI5NiI7czoxMjoiTVVUVUNPUkcyMjk2IjtzOjEyOiJNVVRVQ09SRzIyOTgiO3M6MTI6Ik1VVFVDT1JHMjI5OCI7czoxMjoiTVVUVUNPUkcyMjk3IjtzOjEyOiJNVVRVQ09SRzIyOTciO3M6MTI6Ik1VVFVDT1JHMjIwMSI7cz');
//                exit;
                
                $codigo_organismo = unserialize(base64_decode($codigo_organismo));
//                debug($codigo_organismo);
//                exit;
                $proveedor_id = unserialize(base64_decode($proveedor_id));
                
                $codigo_organismo = (!empty($codigo_organismo) ? "'".implode("','",$codigo_organismo)."'" : "");
...
//                exit;
        ...
        $excel_params = unserialize(base64_decode($asinc->getParametro('txt1')));
		
		
		$liquidaciones	= $this->Liquidacion->liquidacionesByPeriodoDesdeHasta($periodo_desde,$periodo_hasta,FALSE,FALSE,$codigo_organismo);
...
//        $proveedor = $oPRV->read(null,$proveedor_id);        
        $set = array();
        $set['sheet_title'] = 'REPORTE_ACUMULADO';
        $set['labels'] = array(
            'A1' => 'LIQUIDACION:',
            'B1' => $oUT->periodo($periodo_desde) . " / " . $oUT->periodo($periodo_hasta),
//            'A2' => 'PROVEEDOR:',
//            'B2' => $proveedor['Proveedor']['razon_social'],
            'A3' => 'ORGANISMO:',
            'B3' => $oUT->globalDato($codigo_organismo),
            'B5' => $excel_params['titulo'],
        );
        $set['columns'] = $excel_params['columnas'];
...
// 							$comision = round($cuota['LiquidacionCuota']['importe_debitado'],2) * $cuota['ProveedorComision']['comision'] / 100;
							...
							$comisionIVA = $cuota['LiquidacionCuota']['comision_cobranza'] * 21 / 100;
							$comisionIVA = round($comisionIVA,2);
							...
							if(!empty($cuota['LiquidacionSocioRendicion']['banco_intercambio'])){
								...
								$bancoIntercambio = $bancoIntercambioCta = "";
								$indicaPago = 0;
								$codigo_status = "NOE";
								$fechaDebito = null;
							...
							$cuota['LiquidacionCuota']['fecha_debito'] = $fechaDebito;
                                                        ...
                                                        $cuota['LiquidacionCuota']['codigo_organismo_desc'] = $this->OrdenDescuentoCuota->GlobalDato('concepto_1',$cuota['PersonaBeneficio']['codigo_beneficio']);
                                                        $cuota['LiquidacionCuota']['empresa'] = $this->OrdenDescuentoCuota->GlobalDato('concepto_1',$cuota['PersonaBeneficio']['codigo_empresa']);
//                                                        $cuota['LiquidacionCuota']['empresa'] .= " | " . substr($cuota['PersonaBeneficio']['turno_pago'], -5);
// 							debug($cuota);							
							$temp['AsincronoTemporal'] = array(
												
									'asincrono_id' => $asinc->id,
									'clave_1' => 'REPORTE_1',
                                                                        'clave_2' => substr($cuota['LiquidacionCuota']['codigo_organismo_desc'], 0,50),
                                                                        'clave_3' => substr($cuota['LiquidacionCuota']['empresa'],0,50),
									...
									'texto_16' => $cuota['LiquidacionCuota']['orden_descuento_id'],
									...
									'texto_19' => substr($cuota['LiquidacionCuota']['periodo_cuota'],-2)."/".substr($cuota['LiquidacionCuota']['periodo_cuota'],0,4),
                                                                        'texto_20' => $cuota['PersonaBeneficio']['cbu'].' ',
...
// 									'texto_14' => $oUT->periodo($calificacion[2],true,"/"),
									...
									'decimal_6' => round($cuota['LiquidacionCuota']['importe_debitado'] - $cuota['LiquidacionCuota']['comision_cobranza'] - $comisionIVA,2),
									'decimal_7' => round($totalReversado,2),
									'decimal_8' => round($cuota['LiquidacionCuota']['importe'],2),
									'decimal_9' => $comisionIVA,
                                                                        'decimal_10' => round($cuota['OrdenDescuento']['importe_solicitado'],2),
...
//							$cuota['LiquidacionCuota']['producto_cuota'] = $this->OrdenDescuentoCuota->GlobalDato('concepto_1',$cuota['LiquidacionCuota']['tipo_producto'])." - ".$cuota['LiquidacionCuota']['tipo_cuota_desc'];
							...
							$cuota['LiquidacionCuota']['tipo_cuota_desc'] = $cuota['TipoCuot']['concepto_1'];
							$cuota['LiquidacionCuota']['producto_cuota'] =$cuota['TipoProd']['concepto_1'] ." - ". $cuota['TipoCuot']['concepto_1'];
...
//								$bancoIntercambio = $this->LiquidacionCuota->getNombreBanco($cuota['LiquidacionSocioRendicion']['banco_intercambio']);
                                ...
                                $descripcionCodigo = $cuota['BancoRendicionCodigo']['descripcion'];
                                $bancoIntercambio = $cuota['BancoIntercambio']['nombre'];
							...
							$temp['AsincronoTemporal'] = array(
									...
									'texto_13' => $calificacion[0],
									'texto_14' => $oUT->periodo($calificacion[2],true,"/"),
                                    'texto_15' => $cuota['TipoOrga']['concepto_1'],
                                    'texto_16' => $cuota['TipoEmpr']['concepto_1'] . (!empty($cuota['LiquidacionTurno']['descripcion']) ? " - ".$cuota['LiquidacionTurno']['descripcion'] : ""),
                                    ...
                                    'decimal_10' => round($cuota['OrdenDescuento']['importe_solicitado'],2),
									'texto_17' => $cuota['PersonaBeneficio']['cbu'].' ',
							);
					...
					if(!empty($reversos)):
						...
						foreach($reversos as $reverso):
							...
							$temp['AsincronoTemporal'] = array(
                                    ...
                                    'texto_13' => $reverso['OrdenDescuentoCobroCuota']['proveedor_razon_social'],
                                    'texto_14' => $liquidacion['Liquidacion']['organismo'],
                                    'texto_15' => $reverso['OrdenDescuentoCobroCuota']['turno'],
									...
									'decimal_4' => $reverso['OrdenDescuentoCobroCuota']['importe_reversado'] - $reverso['OrdenDescuentoCobroCuota']['comision'],
							         'decimal_5' => $reverso['OrdenDescuentoCobroCuota']['importe'],
							         'entero_1' => $liquidacion['Liquidacion']['periodo'],
						             'entero_2' => $reverso['OrdenDescuentoCobroCuota']['orden_descuento_id'],
							         'texto_16' => ($reverso['OrdenDescuentoCobroCuota']['importe'] == $reverso['OrdenDescuentoCobroCuota']['importe_reversado'] ? 'TOTAL' : 'PARCIAL')
							);						
...
//								}				
        ...
        ###################################################################################################################
        $asinc->actualizar(98,100,"CREANDO ARCHIVO " . $excel_params['file_name'] . " . Espere por favor.");
        $this->Temporal->saveToXLSFile($excel_params['file_name']);                                                
        ###################################################################################################################
        $asinc->fin();
        return;
...
//        ORDER BY Persona.apellido ASC,  Persona.nombre ASC,  LiquidacionCuota.orden_descuento_id ASC,  LiquidacionCuota.periodo_cuota ASC         
		...
		if(!empty($proveedor_id))$conditions['LiquidacionCuota.proveedor_id'] = explode(",",$proveedor_id);
...
//                                'type' => 'left',
//                                'foreignKey' => false,
//                                'conditions' => array('GlobalDato.id = PersonaBeneficio.codigo_empresa')
//                                ),                    
                        array(
                ...
                'fields' => array(
                            ...
                            OrdenDescuento.numero,
                            OrdenDescuento.cuotas,
                            OrdenDescuento.importe_solicitado,
                            OrdenDescuentoCuota.nro_cuota,
                            OrdenDescuentoCuota.importe,
                            OrdenDescuentoCuota.codigo_comercio_referencia,
                            OrdenDescuentoCuota.nro_referencia_proveedor,
...
//	}	
    ...
    function getDetalleNoCobradosDiscriminado($liquidacion_id,$proveedor_id){
        ...
        $sql = "SELECT 
                ...
                INNER JOIN global_datos TipoProd ON (TipoProd.id = LiquidacionCuota.tipo_producto)
                INNER JOIN global_datos TipoCuot ON (TipoCuot.id = LiquidacionCuota.tipo_cuota)
                ...
                INNER JOIN bancos BancoIntercambio ON (BancoIntercambio.id = LiquidacionSocioRendicion.banco_intercambio)
                INNER JOIN banco_rendicion_codigos BancoRendicionCodigo ON (
                ...
                INNER JOIN global_datos TipoOrga ON (TipoOrga.id = PersonaBeneficio.codigo_beneficio)
                INNER JOIN global_datos TipoEmpr ON (TipoEmpr.id = PersonaBeneficio.codigo_empresa)
                INNER JOIN liquidacion_turnos LiquidacionTurno ON (
                LiquidacionTurno.codigo_empresa = PersonaBeneficio.codigo_empresa
                AND LiquidacionTurno.turno = PersonaBeneficio.turno_pago    
                )                
                WHERE 
                LiquidacionCuota.liquidacion_id = $liquidacion_id
                ".(!empty($proveedor_id) ? "AND LiquidacionCuota.proveedor_id IN ($proveedor_id) " : " ")."
                AND (LiquidacionCuota.saldo_actual - LiquidacionCuota.importe_debitado) > 0
                GROUP BY GlobalDato.concepto_1,
                Persona.documento,
                Persona.apellido,
                Persona.nombre,
                Persona.sexo,
