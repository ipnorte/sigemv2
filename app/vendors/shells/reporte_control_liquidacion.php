 ...
 * /usr/bin/php5 /home/adrian/Trabajo/www/sigemv2/cake/console/cake.php reporte_control_liquidacion 56812 -app /home/adrian/Trabajo/www/sigemv2/app/
 * 
...
class ReporteControlLiquidacionShell extends Shell {
...
//				$this->Temporal->setErrorMsg("PASO POR ACA $i",$proveedor['Proveedor']['razon_social_resumida']);
				...
				foreach($cuotas as $cuota){
					...
					$conditions = array();
					$conditions['LiquidacionCuota.liquidacion_id'] = $this->liquidacion['Liquidacion']['id'];
					...
					$conditions['LiquidacionCuota.periodo_cuota'] = $this->liquidacion['Liquidacion']['periodo'];
...
//					$this->out("\tTOTAL : $totalLiqui1 -----> acumulado: $acu");
					...
					$temp['AsincronoTemporal'] = array(
                            ...
                            'entero_3' => $cantidadCobrados,
                                        ...
                                        $asinc->actualizar($i,$total,"$i / $total - PROCESANDO >> " . $proveedor['Proveedor']['razon_social_resumida'] ." - ".$producto['tipo_producto_desc'] . " | " . $cuota['tipo_cuota_desc']);	
...
//				$comisionesReversadas = 0;
				...
				$comisionesReversadas = $IVA_comisionesReversadas = 0;
				...
				$temp['AsincronoTemporal'] = array(
						...
						'decimal_12' => round(($totalReversado + $comisionesReversadas + $IVA_comisionesReversadas)* -1,2),
						'decimal_13' => 0,
						'decimal_14' => round($totalReversado,2),	
						'decimal_15' => round($IVA_comisionesReversadas,2),
						'entero_1' => $proveedor['Proveedor']['id'],
						'entero_2' => $intercambio,
				);
...
// 		}			
	...
	function __cargarProveedores(){
		...
		$proveedores = $this->LiquidacionCuota->find('all',array(
															'conditions' => array('LiquidacionCuota.liquidacion_id' => $this->liquidacion['Liquidacion']['id']),
	...
	function __cargarProductosByProveedor($proveedor_id){
		...
		$registros = $this->LiquidacionCuota->find('all',array(
											...
											'conditions' => array(
																'LiquidacionCuota.liquidacion_id' => $this->liquidacion['Liquidacion']['id'],
																'LiquidacionCuota.proveedor_id' => $proveedor_id
											),
	...
	function __cargarCuotasByProductoByProveedor($proveedor_id,$tipo_producto){
		...
		$registros = $this->LiquidacionCuota->find('all',array(
											...
											'conditions' => array(
																'LiquidacionCuota.liquidacion_id' => $this->liquidacion['Liquidacion']['id'],
																'LiquidacionCuota.proveedor_id' => $proveedor_id,
																'LiquidacionCuota.tipo_producto' => $tipo_producto,
											),
											'fields' => array('LiquidacionCuota.tipo_cuota'),
											'group' => array('LiquidacionCuota.tipo_cuota'),
											'order' => array('GlobalDato.concepto_1')
		));
...
// 																'type' => 'left',
// 																'foreignKey' => false,
// 																'conditions' => array('LiquidacionCuota.orden_descuento_id = OrdenDescuento.id')
// 																),																																																				
// 											),
// 											'conditions' => array(
// 																'LiquidacionCuota.liquidacion_id' => $this->liquidacion['Liquidacion']['id'],
// 																'LiquidacionCuota.proveedor_id' => $proveedor_id,
// 																'LiquidacionCuota.tipo_producto' => $tipo_producto,
// 																'LiquidacionCuota.tipo_cuota' => $tipo_cuota
// 											),
// 											'fields' => array(
