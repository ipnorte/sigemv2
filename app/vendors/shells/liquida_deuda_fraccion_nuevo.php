 ...
 * 	/usr/bin/php5.6 /home/adrian/trabajo/www/sigem/cake/console/cake.php liquida_deuda_fraccion_nuevo 31583 -app /home/adrian/trabajo/www/sigem/app/
...
class LiquidaDeudaFraccionNuevoShell extends Shell {
...
//        $oASINC = &ClassRegistry::init(array('class' => 'Shells.Asincrono', 'alias' => 'Asincrono'));
        ...
        ##################################################################################################################
        $oASINC->actualizar(1, 100, "ESPERE, INICIANDO PROCESO II...");
...
//        $proveedoresIDs = $oASINC->GlobalDato("concepto_2", "MUTUMUTUREPR");
        ...
        while (true) {    
            ...
            $CMD = $oASINC->get_phpcli() . " $CORE liquida_deuda_fraccion_nuevo_lote " . $this->liquidacionID . " " . $asincrono['Asincrono']['id'] . " $SHELL_PID $i $limit $organismo $periodo $pre_imputacion $LIQUIDA_DEUDA_CBU_SP $liqSiTieneDeuda $CONTROL_NACION $BANCO_CONTROL -app ". APP_PATH;
            
            $asincrono['Asincrono']['txt1'] = $CMD;
            $asincrono['Asincrono']['shell_pid'] = $SHELL->background($CMD);
            $SUBASINCRONO->save($asincrono);
            
            $oASINC->actualizar(10, $cantidad, "II CREANDO SUBPROCESO #" . $asincrono['Asincrono']['shell_pid'] . " [$i/$cantidad]");
            
            $i += $limit;
            
            if(!$SHELL->is_running($SHELL_PID)){
//                $SHELL->kill($shellsIds);
                $STOP = TRUE;
                break;
            }
        ...
        if($STOP){
            ...
            foreach ($procesos as $proceso){
                ...
                array_push($shellsIds, $proceso['Asincrono']['shell_pid']);
